...
// ── centralized runtime error table ─────────────────────────
enum {
    ERR_TYPE_MISMATCH = 1,
    ERR_NOT_CALLABLE = 2,
    ERR_INDEX_OOB = 3,
...
// ── obfuscated-build file resolution ───────────────────────
// In obfuscated builds the binary stores only a 64-bit FNV-1a hash token of
// the source file's basename (never a real path). On error we scan the current
// directory for a regular file whose basename hashes to the token, so the
// traceback can still show source lines without the binary leaking any path.
...
// Returns 1 and fills `out` with a filename in the current directory whose
// basename hashes to `token` (a 16-hex-char FNV-1a token). Returns 0 otherwise.
...
// Resolves a stored filename (a hash token in obfuscated builds, otherwise a
// real path) to a path that exists, for the error traceback source display.
// Returns the resolved path (in `buf`/`buflen`) if one exists, else `input`.
...
// Decrypts an XOR-encrypted string constant (len bytes, key) into a string object.
